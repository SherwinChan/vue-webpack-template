...
let ajaxUrl = 'http://yimentong.dev.sj33333.com/index.php'; //默认的api地址
...
if (env === 'development') {
  ...
  if (server) {
    let url = 'http://yimentong.' + server + '.sj33333.com/index.php';  //切换的api地址
    store.commit('setServer', server) //记录server进vuex
    ajaxUrl = url;
  } else {
    // 若无server选项，则本地开发填默认api地址，线上则默认服务器域名地址
    {{/vuex}}ajaxUrl = window.location.hostname == 'localhost' ? 'http://yimentong.master.sj33333.com/index.php' : window.location.origin + '/index.php';
    {{#vuex}}
  }{{/vuex}}
} else {
  if (env === 'production') {
